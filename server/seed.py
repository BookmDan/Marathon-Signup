...
from config import app, db
from models.models import User, RaceSignup, RaceEvent, CreditCardInformation
...
if __name__ == "__main__":
    with app.app_context():
        ...
        db.session.query(CreditCardInformation).delete()
        db.session.commit()
        ...
        # Create race events
        race_event1 = RaceEvent(organization="BigHearts", race_type="5k")
        race_event2 = RaceEvent(organization="10000Runs", race_type="10k")
        db.session.add_all([race_event1, race_event2])
        db.session.commit()

        # Create credit card information
        credit_card1 = CreditCardInformation(credit_card_number="1234567890123456", name_on_card="John Doe", expiration_date="12/25", cvv="123", street_address="789 Elm St", country="US", zipcode="67890", city="Villagetown", state="VT", save_my_card=True)
        credit_card2 = CreditCardInformation(credit_card_number="9876543210987654", name_on_card="Jane Doe", expiration_date="06/23", cvv="456", street_address="321 Pine St", country="US", zipcode="54321", city="Towndale", state="TS", save_my_card=False)
